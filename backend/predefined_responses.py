"""
System of predefined responses for the director based on keywords.
Allows quick responses to standard questions without calling AI.
"""
...
# Dictionary of keywords for main investigation topics
KEYWORD_PATTERNS = {
    "christmas_card": {
    ...
    "arrival_time": {
        "keywords": ["arrived", "arrive", "arrival",  "how did you get", "come to", "came to", "get here", "get to alex", "transport", "transportation"],
...
def extract_character_from_message(message: str) -> Optional[str]:
    """Extracts character name from message, including various forms of address."""
    message_lower = message.lower()
    ...
    # Search anywhere in message (old logic for predefined responses)
...
def extract_character_from_message_strict(message: str) -> Optional[str]:
    """Strictly determines direct addresses to characters, excluding random mentions."""
    ...
    # Pattern 1: "Tim, question..." - name at start with comma
    for char_key, names in character_names.items():
        for name in names:
            if message_lower.startswith(f"{name},"):
                return char_key
    
    # Pattern 2: "question..., Tim?" - name at end with comma
    for char_key, names in character_names.items():
        for name in names:
            if message_lower.endswith(f", {name}?") or message_lower.endswith(f", {name}"):
                return char_key
    
    # Pattern 3: "Tim what/where/when/why/how..." - name at start + question word
    ...
    for char_key, names in character_names.items():
        for name in names:
            if message_lower.startswith(f"{name} "):
                # Check if there's a question word after the name
...
def _mark_predefined_as_used(user_id: int, topic_key: str):
    """
    Marks predefined response as used in game state.
    """
    from config import GAME_STATE
...
def create_predefined_response(topic_key: str, character_keys: List[str], topic_memory: Dict) -> Dict[str, Any]:
    """
    Creates a predefined response-scene for one or more characters.
    """
    if topic_key not in KEYWORD_PATTERNS:
        return {"scene": []}
...
def try_predefined_response(user_id: int, message: str, topic_memory: Dict) -> Optional[Dict[str, Any]]:
    ...
    # Check if predefined response was already used for this topic
    predefined_used = topic_memory.get("predefined_used", [])
    ...
    # If topic changed, create clean memory for new topic
    current_topic = topic_memory.get("topic", "None")
    
    if current_topic != topic_name:
        # New topic - reset spoken list but keep predefined_used
    ...
    # Check if there's a specific character in the message
    specific_character = extract_character_from_message(message)
    
    if specific_character:
        # Player addresses specific character
    ...
    # Player asks general question - select all suitable characters for response
    ...
    if not available_characters:
        # All characters already spoke on this topic, but player might ask new related question
        # Let director decide whether to respond or move to another topic
        print(f"DEBUG PREDEFINED: All characters have spoken on topic {detected_topic}, letting director decide")
        return None
    
    # Select characters for response based on strategy
