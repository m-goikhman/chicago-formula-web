...
**Шаг 2: Деплой**
```bash
cd ../frontend  # Перейти в директорию фронтенда
...
После подключения кастомного домена нужно добавить его в список разрешенных origins в `backend/main.py`:
