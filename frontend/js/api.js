...
async function sendMessage() {
    ...
    try {
        const response = await fetch(`${API_URL}/api/game/message`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${sessionToken}`
        ...
        // Handle multiple messages from response
        if (data.messages && Array.isArray(data.messages)) {
            await displayMessagesSequentially(data.messages);
        ...
        // Remove typing indicator after all messages are displayed
...
async function explainWord(wordOrPhrase, originalText) {
    try {
        const response = await fetch(`${API_URL}/api/game/explain`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${sessionToken}`
        ...
        if (data.messages && data.messages.length > 0) {
            // Clear selection after successful explanation
